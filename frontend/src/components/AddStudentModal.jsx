...
export default function AddStudentModal({ isOpen, onClose, courseId, courseName, isExternalCourse = false, onStudentAdded }) {
    ...
    const fetchProvidersAndTracks = async () => {
        try {
            const pRes = await fetch(`/api/training/providers/list.php?course_id=${courseId}`, { credentials: 'include' });
            const pData = await pRes.json();
            if (pRes.ok && pData.providers) {
                setProviders(pData.providers);
            }

            const tRes = await fetch(`/api/training/topics/list.php?course_id=${courseId}`, { credentials: 'include' });
    ...
    const fetchCandidates = async (q = '') => {
        ...
        try {
            const res = await fetch(`/api/training/enrollments/search_candidates.php?course_id=${courseId}&q=${encodeURIComponent(q)}`, { credentials: 'include' });
        ...
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-box add-student-modal animate-scale-in" onClick={e => e.stopPropagation()}>
                ...
                {isExternalCourse ? (
                    ...
                    tracks.length > 0 && (
                        <div style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: '10px', padding: '0.75rem 1rem', marginBottom: '1.25rem' }}>
                            <label style={{ fontSize: '0.8rem', color: 'var(--text-2)', display: 'block', marginBottom: '4px', fontWeight: 600 }}>
                                {lang === 'ar' ? 'المسار التدريبي (اختياري):' : 'Training Track (Optional):'}
                            </label>
                            <select
                                className="form-control"
                                style={{ width: '100%', padding: '0.45rem 0.65rem', fontSize: '0.85rem', borderRadius: '6px', background: 'var(--bg-1)', color: 'var(--text-1)', border: '1px solid var(--border)' }}
